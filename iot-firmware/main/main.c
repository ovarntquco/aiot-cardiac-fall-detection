...
void app_main() {
    ...
    err = network_prov_init();
    
    if (ESP_OK != err) {
        ESP_LOGE(NETWORKPROV_TAG, "Failed to init Network Provisioning: %s", esp_err_to_name(err));
        return;
    }
    ...
    if (ESP_OK != err) {
        ESP_LOGE("TIME_SYNC", "Failed to sync time: %s", esp_err_to_name(err));
        return;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE("BUS_TAG", "Failed to create I2C bus: %s", esp_err_to_name(err));
        return;
    }
